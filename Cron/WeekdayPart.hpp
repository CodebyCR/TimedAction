...
private:
     ...
     */
    [[nodiscard]] auto to_weekday(std::string& weekday) const -> int {
...
public:
    ...
    WeekdayPart(std::string weekdays) : rawValue(std::move(weekdays)) {
        process();
    }
    ...
    [[nodiscard]] auto getContainedWeekdays() const -> std::vector<int> const& {
